classdef arm_end_effector_flat_RRT_star_HLP < RRT_star_HLP
    properties
        start = zeros(3,1) ;
        buffer = 0 ;
    ...
    methods
        %% constructor
        function HLP = arm_end_effector_flat_RRT_star_HLP(varargin)
...
%             q_0 = HLP.goal_configuration ;
            q_0 = 0.7 * agent_info.state(agent_info.joint_state_indices,end) + ...
                  0.3 * HLP.goal_configuration;
            
            % inverse kinematics to get q close to z
            HLP.vdisp('Running inverse kinematics to get waypoint')
            [q,exitflag] = agent_info.inverse_kinematics_flat_end_effector(z,q_0) ;
